...
file_path = os.path.join(
    '/home/wenqi/GL-SMART-aim2/deep_model_weighted_loss_MCID_fair/deep-models-{}/'.format(args.time_len), focus_label+'/')
...
data_path = '/home/wenqi/GL-SMART-aim2/pre-processed-data-MCID/pp_data_preop_{}.csv'.format(
    args.time_len)
models_folder = '/home/wenqi/GL-SMART-aim2/deep_model_weighted_loss_MCID_fair/deep-models-{}/{}/'.format(
    args.time_len, focus_label)
tensorboard_folder = '/home/wenqi/GL-SMART-aim2/deep_model_weighted_loss_MCID_fair/deep-models-{}/{}/tensorboard/'.format(
    args.time_len, focus_label)

# Create results spreadsheet if it does not already exist
results_file = '/home/wenqi/GL-SMART-aim2/deep_model_weighted_loss_MCID_fair/deep-models-{}/{}/results/Validation-80Trained_HPTuneSpreadsheet.csv'.format(
    args.time_len, focus_label)
...
model_name = 'fairsample_ALLFEATURES_ACTIVATION={}_CN={}_D1P={}_D2P={}_D3P={}_LR={}_OUTCOME={}'.format(
...
## Loss and Optimizer ##
loss = nn.BCELoss(reduce=False)  # reduction='sum')
...
x_train_male = x_train_val[x_train_val['Gender'].values < 0]
y_train_male = y_train_val[x_train_val['Gender'].values < 0]
x_train_female = x_train_val[x_train_val['Gender'].values > 0]
y_train_female = y_train_val[x_train_val['Gender'].values > 0]
print(len(x_train_male), len(x_train_female))
new_x_train_val = pd.DataFrame(np.concatenate(
    (np.repeat(x_train_male.values, 10, axis=0), x_train_female), axis=0))
new_y_train_val = pd.DataFrame(np.concatenate(
    (np.repeat(y_train_male.values, 10, axis=0), y_train_female), axis=0))
print(len(new_x_train_val), len(new_y_train_val))
x_train_val = new_x_train_val
y_train_val = new_y_train_val
## Stratified Split ##
...
print('Train Dataset: %s %s' % (x_train.shape, y_train.shape))
...
print('Valid Dataset: %s %s' % (x_test.shape, y_test.shape))
############################################# TRAINING #################################################
########################################################################################################
## Tensorboard ##
...
# ## Iterate through thresholds from 0-1 to calculate TPR and FPR to generate data points for ROC curve
# thresholds = np.arange(0,1+1e-13,0.001)
# tpr = list()
# fpr = list()
# for threshold in thresholds:
#     tn, fp, fn, tp = confusion_matrix(label_true, label_pred>threshold).ravel()
#     tpr.append( tp/(tp+fn) )
#     fpr.append( fp/(fp+tn) )
...
# plt.title(f'AUROC Curve = {round(roc_auc_score(label_true, label_pred),3)}', fontsize=20);
# plt.xlabel('False Positive Rate (FPR)', fontsize=16); plt.ylabel('True Positive Rate (TPR)', fontsize=16);

# ## Save image and create corresponding Tensorboard file
# plt.savefig(model_name + 'TEMP_auroc_curve.jpg')
# img_array_auroc = np.array(PIL.Image.open(model_name + 'TEMP_auroc_curve.jpg')).transpose((2,0,1))
# tb.saveImage(img_array_auroc, 'AUROC Curve')
...
# ## Plot Sensitivity/Specificity versus different thresholds
# plt.figure(figsize=(10,6))
# plt.plot(thresholds, sensitivity_array)
# plt.plot(thresholds, specificity_array)
# plt.title('Sensitivity and Specificity vs Threshold', fontsize=24);
# plt.xlabel('Threshold', fontsize=20); plt.ylabel('Sensitivity / Specificity', fontsize=20); plt.xlim([0, 1]);
# plt.legend(['Sensitivity', 'Specificity'], fontsize=16)

# ## Save image and create corresponding Tensorboard file
# plt.savefig(model_name + 'TEMP_sensitivity_specificity_curve.jpg')
# img_array_sensitivity_specificity = np.array(PIL.Image.open(model_name + 'TEMP_sensitivity_specificity_curve.jpg')).transpose((2,0,1))
# tb.saveImage(img_array_sensitivity_specificity, 'Sensitivity/Specificity Curve')


############################################# MCC vs Threshold #############################################
# ## MCC at different thresholds
# thresholds = np.arange(0,1+1e-13,0.001)
# mcc_array = list()
# for threshold in thresholds:
#     tn, fp, fn, tp = confusion_matrix(label_true, label_pred>threshold).ravel()
...
#     else:
#         mcc_array.append( matthews_corrcoef(label_true, label_pred>threshold) )

# max_mcc = np.max(np.array(mcc_array)[~np.isnan(mcc_array)]) # maximum MCC value across all thresholds
# best_threshold = thresholds[int(np.median(np.where(np.array(mcc_array) == max_mcc)))] # threshold at which we achieve max MCC

# ## Plot MCC versus different thresholds
# plt.figure(figsize=(10,4))
# plt.plot(thresholds, mcc_array)
# plt.title(f'MCC vs Threshold (Maximum MCC = {round(max_mcc, 3)} @ Threshold = {round(best_threshold, 3)})');
# plt.xlabel('Threshold'); plt.ylabel('MCC'); plt.xlim([0, 1]);

# ## Save image and create corresponding Tensorboard file
# plt.savefig(model_name + 'TEMP_mcc_vs_threshold.jpg')
# img_array_mcc_threshold = np.array(PIL.Image.open(model_name + 'TEMP_mcc_vs_threshold.jpg')).transpose((2,0,1))
# tb.saveImage(img_array_mcc_threshold, 'MCC vs Threshold')


############################################# Confusion Matrix for Best Threshold #############################################
# import dataframe_image as dfi
# best_threshold = thresholds[int(np.median(np.where(np.array(mcc_array) == np.max(np.array(mcc_array)[~np.isnan(mcc_array)]))))]
# best_confusion_matrix = confusion_matrix(label_true, label_pred > best_threshold)
# best_confusion_matrix_df = pd.DataFrame(best_confusion_matrix, index=['Actual: Negative', 'Actual: Positive'],
#                                         columns=['Predicted: Negative', 'Predicted: Positive'])

# ## Save image and create corresponding Tensorboard file
# dfi.export(best_confusion_matrix_df, model_name + "TEMP_best_confusion_matrix.jpg", table_conversion='matplotlib')
# img_array_confusion_matrix = np.array(PIL.Image.open(model_name + 'TEMP_best_confusion_matrix.jpg')).transpose((2,0,1))
# tb.saveImage(img_array_confusion_matrix, 'Best Confusion Matrix')


############################################# Cleanup #############################################
try:
    os.remove(model_name + 'TEMP_auroc_curve.jpg')
except:
    pass
try:
    os.remove(model_name + 'TEMP_sensitivity_specificity_curve.jpg')
except:
    pass
try:
    os.remove(model_name + 'TEMP_mcc_vs_threshold.jpg')
except:
    pass
try:
    os.remove(model_name + 'TEMP_best_confusion_matrix.jpg')
except:
    pass
